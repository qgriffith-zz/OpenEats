...
def fav_link(user, recipe_id):
    ...
    if  user.is_authenticated(): #make sure the user is signed in
        ...
        if check:
            return "<a class=\"btn-inverse\" href=\"#\">%s</a>" % translation.ugettext('bookmakred')
        else: #must not be stored yet
            return "<a class=\"btn-info\" id=\"recipe-store\">%s</a>" % translation.ugettext('favorite')
...
def report_link(user, recipe_id):
    ...
    if  user.is_authenticated(): #make sure the user is signed in
        ...
        if check:
            return "<a class=\"btn-danger disabled\" id=\"recipe-report\" href=\"#\" title=\"recipe has been reported as spam\">%s</a>" % translation.ugettext('reported!')
        else: #must not been reported yet yet
            return "<a class=\"btn-info\" id=\"recipe-report\" title=\"report inappropriate recipe to the admins\">%s</a>" % translation.ugettext('report')
